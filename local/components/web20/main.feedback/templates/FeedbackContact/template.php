...
<div class="mfeedback">
    ...
    <form id="contact-form" class="default-form" action="<?= POST_FORM_ACTION_URI ?>" method="POST">
        ...
        <div class="mf-email col-md-4 col-sm-6">
            <div class="mf-text">
                <?= GetMessage("MFT_EMAIL") ?><? if (empty($arParams["REQUIRED_FIELDS"]) || in_array("EMAIL", $arParams["REQUIRED_FIELDS"])): ?>
                    <span class="mf-req">*</span><? endif ?>
            </div>
            <input name="user_email" value="<?= $arResult["AUTHOR_EMAIL"] ?>" type="email" placeholder="Email"/>
        </div>
        <div class="mf-phone col-md-4 col-sm-12">
            <div class="mf-text">
                <?= GetMessage("MFT_PHONE") ?><? if (empty($arParams["REQUIRED_FIELDS"]) || in_array("PHONE", $arParams["REQUIRED_FIELDS"])): ?>
                    <span class="mf-req">*</span><? endif ?>
            </div>
            <input name="user_phone" value="<?= $arResult["AUTHOR_PHONE"] ?>" type="text" placeholder="Телефон"/>
        </div>
        <div class="mf-message col-md-12 col-sm-12">
            <div class="mf-text">
